...
exports.errorHelper = function errorHelper(err, cb) {

  //If it isn't a mongoose-validation error, just throw it.
  if (err.name !== 'ValidationError') if(cb) return cb(err); else return err;
  var messages = {
  ...
  if(cb) return cb(errors); else return errors;
}
